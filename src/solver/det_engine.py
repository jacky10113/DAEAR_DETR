...
def evaluate(model: torch.nn.Module, criterion: torch.nn.Module, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        for target, result in zip(targets, results):
            ...
            for it in result['labels']:
              ...
              bbox=result['boxes'][n].tolist()
              #if score>=0:
              item={"image_id": image_id, "category_id": 1, 
                      "bbox": [bbox[0],bbox[1],bbox[2]-bbox[0],bbox[3]-bbox[1]],#转换成xywh
                        "score": score}
    ...
    result_file_path = os.path.join(output_dir, "DAEAR-DETR_predictions.json")
