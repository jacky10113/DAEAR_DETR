...
def main():
  ...
  for i in range(im_data.shape[0]):
      ...
      for b in box:
        ...
        #bboxes_txt.append([bbox])
        x, y, x_max, y_max=bbox
        bboxes_txt.append([x, y, x_max, y_max])
  ...
  if args.save_txt:
     #fmt = '[' + ','.join(['%f']*bboxes_txt) + '],'
     #np.savetxt('bboxes.txt',bboxes_txt,fmt='%f', delimiter=', ', comments='', newline='\n')
     with open('bboxes.txt', 'w') as file:
      for item in bboxes_txt:
        line = ', '.join(map(str, item))  # 将每个数字转换为字符串并用逗号分隔
        file.write(f'[{line}],\n')  # 将行写入文件，每个列表项作为一行
